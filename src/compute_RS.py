...
def compute_RS(grid):
    ...
    R_uu = (grid.u - roll_view(grid.u,-1,axis=0)) / grid.dx
    ...
    R_vv = (grid.v - roll_view(grid.v,-1,axis=1)) / grid.dy
    R_vw = 0.5*((grid.v - roll_view(grid.v,-1,axis=2)) / grid.dz - (grid.w - roll_view(grid.w,-1,axis=1)) / grid.dy)
    R_wu = 0.5*((grid.w - roll_view(grid.w,-1,axis=1)) / grid.dx - (grid.w - roll_view(grid.w,-1,axis=1)) / grid.dz)
    R_wv = 0.5*((grid.w - roll_view(grid.w,-1,axis=1)) / grid.dy - (grid.w - roll_view(grid.w,-1,axis=1)) / grid.dz)
    R_ww = (grid.w - roll_view(grid.w,-1,axis=2)) / grid.dz
