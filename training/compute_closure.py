...
def dtau_del(tau, delta, grid_spacing):
        ...
        dtauijdx1 = dtauijdx1[1,1,1]
        dtauijdx2 = dtauijdx2[1,1,1]
        dtauijdx3 = dtauijdx3[1,1,1]
...
class nu_deriv_funct(torch.autograd.Function):
        @staticmethod
        def forward(ctx, input_data, S, delta, grid_spacing, tau_act):
                tau = -2 * input_data * S[1,1,1]
                ctx.save_for_backward(torch.tensor(grid_spacing, dtype=torch.float32), torch.tensor(S, dtype=torch.float32))
                pred_tau = cp.deepcopy(tau_act)
                pred_tau[1,1,1] = tau
                return dtau_del(pred_tau, delta, grid_spacing)
        
        @staticmethod
        def backward(ctx, grad_output):
                # d closure_k / d tau_ij
                grid_spacing = ctx.saved_tensors[0].detach().numpy()
                S = ctx.saved_tensors[1]
                delta = torch.zeros([3,1])
                dnudclose = -2 * dtau_del(S, delta, grid_spacing)
